...
import { SUPPORTED_ASPECT_RATIOS, SUPPORTED_IMAGE_SIZES } from './types/index.js';
import type { AspectRatio, ImageSize } from './types/index.js';
...
const SUPPORTED_ASPECTS_SET = new Set<string>(SUPPORTED_ASPECT_RATIOS);
const SUPPORTED_SIZES_SET = new Set<string>(SUPPORTED_IMAGE_SIZES);
...
function printGenerateHelp() {
  console.log(
    [
      ...
      '  -a, --aspect <ratio>         Aspect ratio: 1:1, 1:4, 1:8, 2:3, 3:2, 3:4, 4:1, 4:3, 4:5, 5:4, 8:1, 9:16, 16:9, 21:9. Defaults to 1:1.',
      '  -z, --size <size>            Image size: 512, 1K, 2K, 4K. Defaults to 1K. 512 is Flash models only.',
      '  -m, --model <id>             Gemini model ID. Default: gemini-3-pro-image-preview.',
      ...
      '',
      'Examples:',
      '  gemini-image generate -p "A brown leather wallet on white background" -a 4:5 --size 1K',
      '  gemini-image generate -p "A watercolor landscape" -a 16:9 --size 2K -m gemini-3-pro-image-preview',
...
function printEditHelp() {
  console.log(
    [
      ...
      '  -a, --aspect <ratio>  Output aspect ratio. Use to change the aspect ratio of the edited image.',
      '  -z, --size <size>     Image size: 512, 1K, 2K, 4K.',
      '  -m, --model <id>      Gemini model ID. Default: gemini-3-pro-image-preview.',
      '  -o, --output <path>   Where to save the edited image. Defaults to current directory.',
      '  -h, --help            Show this help message.',
      '',
      'Example:',
      '  gemini-image edit -p "Extend the background for a taller composition" -i ./image.png -a 4:5 -o ./output.png',
    ].join('\n')
  );
}
...
function parseGenerateCommand(args: string[]): GenerateCommandOptions {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg.startsWith('--size=')) {
      options.imageSize = parseImageSize(arg.slice('--size='.length));
      continue;
    }
    if (arg === '--size' || arg === '-z') {
      const { value, nextIndex } = requireOptionValue(args, i, '--size');
      options.imageSize = parseImageSize(value);
      i = nextIndex;
      continue;
    }
    ...
    if (arg === '--model' || arg === '-m') {
      const { value, nextIndex } = requireOptionValue(args, i, '--model');
      options.model = value;
      i = nextIndex;
      continue;
    }
...
function parseAspect(value: string): AspectRatio {
  if (!SUPPORTED_ASPECTS_SET.has(value)) {
    throw new Error(
      `Invalid aspect ratio: ${value}. Expected one of: ${SUPPORTED_ASPECT_RATIOS.join(', ')}.`
    );
  }
...
function parseImageSize(value: string): ImageSize {
  if (!SUPPORTED_SIZES_SET.has(value)) {
    throw new Error(
      `Invalid image size: ${value}. Expected one of: ${SUPPORTED_IMAGE_SIZES.join(', ')}.`
    );
  }
